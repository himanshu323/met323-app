...
export class Actions {
    ...
    **/
    ...
    static async getSingleSelectValues(placeHolderValue, ...values) {
        ...
        let selctElementToClick = await element(by.xpath(`//mat-option//span[@class='mat-option-text'][text()='${values[0].toUpperCase()}']`));

        await browser.executeScript('arguments[0].click()', selctElementToClick);
//let selectElement = await element(by.xpath(`//mat-option//span[@class='mat-option-text'][text()='${value.toUpperCase()}']`));

    }

static async multiSelectFromGeneralTab(placeHolderValue, ...values) {
